...
export class Shipment implements IShipment {
    ...
    static getShipment = async (id: string): Promise<any> => {
        try {
            const promisePool = pool.promise();
            const rows = await promisePool.execute('SELECT s.id, s.status, s.date, u.name, op.order_id, op.product_id, op.shipment_id, op.quantity, op.price FROM `shipment` s JOIN `order_product` op ON op.shipment_id=s.id JOIN `order` o on op.order_id=o.id JOIN user u on o.user_id=u.id WHERE s.id = ?', [id]);
            return rows[0];
        } catch (error) {
            return { error: error }
        }
